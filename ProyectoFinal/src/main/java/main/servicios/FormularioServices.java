...
public class FormularioServices {
    ...
    public List<Formulario> listarFormulariosPorUsuarioPaginado(String username, int page, int size) {
        ...
        try (MorphiaCursor<Formulario> cursor = ds.find(Formulario.class).filter(eq("usuario.username", username)).iterator(options)) {
