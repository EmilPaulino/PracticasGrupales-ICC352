...
public class ArticuloController {
    ...
    * */
    public void crear(Context ctx) {
        ...
        Articulo existe = articuloService.buscarPorTitulo(titulo);

        if (existe != null) {
            ctx.attribute("error", "Ya existe un artículo con ese título.");
            ctx.attribute("etiquetas", etiquetaService.findAll());
            ctx.render("templates/articulos/formularioArticulo.html");
            return;
        }
    ...
    * */
    public void editar(Context ctx) {
        ...
        Articulo existente = articuloService.buscarPorTitulo(nuevoTitulo);

        if (existente != null && existente.getId() != articulo.getId()) {
            ctx.attribute("error", "Ya existe un artículo con ese título.");
            String etiquetasTexto = articulo.getListaEtiquetas()
                    .stream()
                    .map(Etiqueta::getEtiqueta)
